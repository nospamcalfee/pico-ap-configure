...
int main() {
  ...
  DEBUG_printf("Standing by for multicore...\n");
  ...
  ip4_addr_t mask;
  IP4_ADDR(ip_2_ip4(&state->gw), 192, 168, 5, 1);
  ...
  while (!state->complete) {
    ...
    // periodically from your main loop (not from a timer interrupt) to
    // check for Wi-Fi driver or lwIP work that needs to be done.
    cyw43_arch_poll();
