 ...
 */
static err_t tcp_json_sending(TCP_CLIENT_T *state, struct tcp_pcb *pcb) {
...
static err_t tcp_json_poll(void *arg, struct tcp_pcb *tpcb) {
    ...
    if (state->status == ERR_MEM) {
        tcp_json_sending(state, tpcb); //retry send
    }
    return ERR_OK;
}
...
err_t tcp_json_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err) {
    ...
    return tcp_json_sending(state, tpcb);
}
